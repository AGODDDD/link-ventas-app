    ...
    async function loadProduct() {
      ...
      if (data.variants && Array.isArray(data.variants)) {
        if (currentTemplateType === 'restaurante') {
           ...
           // Tallas — igual que antes
           const tSet = new Set(data.variants.map((v: any) => v.talla).filter(Boolean))
           setTallasInput(Array.from(tSet).join(', '))
           
           // Colores — preservar image_url por color
           const coloresMap = new Map<string, string | undefined>()
           data.variants.forEach((v: any) => {
             if (v.color && !coloresMap.has(v.color)) {
               coloresMap.set(v.color, v.image_url || undefined)
             }
           })
           setColoresList(
             Array.from(coloresMap.entries()).map(([color, image_url]) => ({
               color,
               ...(image_url ? { image_url } : {})
             }))
           )
