...
import { CalendarDays, CheckCircle2, Copy, CreditCard, Loader2, Mail, Pencil, ShieldCheck, Trash2, UserRound } from 'lucide-react'
...
export default function AccountCenter() {
  ...
  // Datos de presentación ya disponibles; las acciones siguen validándose en servidor.
  const [account, setAccount] = useState<Account>(() => ({
    id: session.userId,
    fullName: session.userDisplayName,
    avatarUrl: session.userAvatarUrl,
    email: session.userEmail,
    provider: session.userProvider,
    createdAt: session.userCreatedAt,
    plan: session.planStatus || 'free',
    planExpiresAt: session.planExpiresAt,
    deletionRequest: null,
  }))
  const [name, setName] = useState(session.userDisplayName)
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) {
          ...
          if (!nameEdited.current) setName(data.account.fullName)
        }
      } catch (error) {
  ...
  const saveName = async (event: FormEvent) => {
    event.preventDefault(); if (loading || loadError) return; const headers = await authHeaders(); if (!headers) return
    setSaving(true)
    try {
