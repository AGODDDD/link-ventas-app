    ...
    <section id="catalogo" className="scroll-mt-16 py-12 lg:py-16">
      <div className="mx-auto max-w-[1480px] px-4 sm:px-6">
        <div className="flex flex-col gap-5 border-b border-zinc-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div><p className="text-[9px] font-bold uppercase tracking-[0.18em] text-[#2563EB]">Catálogo actualizado</p><div className="mt-2 flex items-end gap-5"><button type="button" onClick={() => setTab('new')} className={`text-2xl font-black tracking-[-0.045em] transition-colors sm:text-3xl ${tab === 'new' ? 'text-[#172033]' : 'text-zinc-300'}`}>Nuevos productos</button><button type="button" onClick={() => setTab('best')} className={`pb-1 text-sm font-bold transition-colors ${tab === 'best' ? 'text-[#172033]' : 'text-zinc-400'}`}>Más vendidos</button></div></div>
          <label className="flex h-11 w-full items-center gap-3 rounded-xl border border-slate-200 bg-white px-3.5 transition-all duration-300 focus-within:border-blue-400 focus-within:shadow-[0_10px_30px_rgba(37,99,235,0.08)] lg:w-80"><AnimatedSearch size={16} duration={0.45} className="text-zinc-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar productos..." className="min-w-0 flex-1 bg-transparent text-xs outline-none placeholder:text-zinc-400" />{query && <button type="button" onClick={() => setQuery('')}><AnimatedClose size={14} className="text-zinc-400" /></button>}</label>
        </div>

        <div className="flex gap-2 overflow-x-auto py-5 [scrollbar-width:none]">{categories.map((item) => <button key={item} type="button" onClick={() => setCategory(item)} className={`shrink-0 rounded-full border px-4 py-2 text-[9px] font-bold uppercase tracking-[0.07em] transition-all duration-300 active:scale-95 ${category === item ? 'border-[#2563EB] bg-[#2563EB] text-white' : 'border-slate-200 bg-white text-zinc-500 hover:border-blue-300 hover:text-[#1D5ED8]'}`}>{item}</button>)}</div>
        ...
        {visible.length ? <div className="group/carousel relative" onMouseEnter={() => setCarouselPaused(true)} onMouseLeave={() => setCarouselPaused(false)}><div ref={carouselRef} data-product-carousel className="flex snap-x snap-mandatory gap-3 overflow-x-auto scroll-smooth pb-5 [scrollbar-width:none]">{visible.map((product) => {
          ...
          return <article key={product.id} className="group relative flex w-[76vw] shrink-0 snap-start flex-col rounded-2xl border border-slate-200/90 bg-white p-3 transition-all duration-300 hover:z-10 hover:-translate-y-1 hover:border-blue-300 hover:shadow-[0_20px_48px_rgba(37,99,235,0.1)] sm:w-[260px] lg:w-[calc((100%-36px)/4)]">
            <div className="relative aspect-square overflow-hidden rounded-xl bg-[#F7F9FC]">{product.image_url ? <Image src={product.image_url} alt={product.name} fill className="object-contain p-2 transition-transform duration-500 group-hover:scale-105" sizes="(max-width:640px) 50vw, 20vw" /> : <span className="flex h-full items-center justify-center text-zinc-200"><AnimatedPackage size={38} /></span>}<div className="absolute left-1 top-1 flex flex-col items-start gap-1">{product.created_at && <span className="bg-[#DBEAFE] px-2 py-1 text-[8px] font-black uppercase tracking-[0.1em] text-[#1D4ED8]">Nuevo</span>}{discount > 0 && <span className="bg-[#F97316] px-2 py-1 text-[8px] font-black text-white">−{discount}%</span>}</div><button type="button" onClick={() => setQuickView(product)} aria-label={`Vista rápida de ${product.name}`} className="absolute right-1 top-1 flex h-8 w-8 items-center justify-center rounded-full border border-zinc-200 bg-white text-zinc-400 opacity-100 shadow-sm transition-all focus:translate-y-0 focus:opacity-100 lg:translate-y-1 lg:opacity-0 lg:group-hover:translate-y-0 lg:group-hover:opacity-100"><AnimatedEye size={15} duration={0.45} /></button></div>
            <div className="mt-2 flex flex-1 flex-col"><p className="text-[8px] font-bold uppercase tracking-[0.12em] text-zinc-400">{product.brand || product.category || 'Producto'}</p><h3 className="mt-1.5 line-clamp-3 min-h-[48px] text-[11px] font-bold leading-4 text-[#315B88] transition-colors group-hover:text-[#172033] sm:text-xs">{product.name}</h3><div className="mt-2 space-y-0.5 text-[9px] text-zinc-400"><p>Stock: <strong className={outOfStock ? 'text-red-500' : 'text-zinc-600'}>{product.stock == null ? 'Disponible' : `${product.stock} ${product.stock === 1 ? 'artículo' : 'artículos'}`}</strong></p>{product.brand && <p>Marca: <strong className="text-zinc-600">{product.brand}</strong></p>}</div><div className="mt-auto pt-3"><div className="flex flex-wrap items-baseline gap-x-2"><strong className="text-sm font-black text-[#172033] sm:text-base">{money(product.price)}</strong>{product.original_price && product.original_price > product.price && <span className="text-[9px] text-zinc-400 line-through">{money(product.original_price)}</span>}</div>
              {!isReadOnly && !outOfStock && !line && <button type="button" onClick={() => add(product)} className="mt-3 flex h-9 w-full items-center justify-center gap-1.5 rounded-lg bg-[#F97316] text-[9px] font-bold uppercase tracking-[0.05em] text-white shadow-[0_8px_18px_rgba(249,115,22,0.18)] transition-all duration-300 hover:bg-[#EA580C] hover:shadow-[0_10px_24px_rgba(249,115,22,0.28)] active:scale-[0.98]"><AnimatedCart size={14} duration={0.45} />Añadir al carrito</button>}
        ...
        })}</div>{visible.length > 2 && <><button type="button" onClick={() => moveCarousel(-1)} aria-label="Productos anteriores" className="absolute -left-3 top-[42%] z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white text-[#172033] opacity-100 shadow-[0_8px_24px_rgba(30,64,175,0.12)] transition-all hover:border-blue-300 hover:text-[#2563EB] active:scale-95 lg:opacity-0 lg:group-hover/carousel:opacity-100"><AnimatedChevronLeft size={18} duration={0.45} /></button><button type="button" onClick={() => moveCarousel(1)} aria-label="Productos siguientes" className="absolute -right-3 top-[42%] z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-slate-200 bg-white text-[#172033] opacity-100 shadow-[0_8px_24px_rgba(30,64,175,0.12)] transition-all hover:border-blue-300 hover:text-[#2563EB] active:scale-95 lg:opacity-0 lg:group-hover/carousel:opacity-100"><AnimatedChevronRight size={18} duration={0.45} /></button></>}</div> : <div className="border border-dashed border-zinc-300 bg-white py-20 text-center"><AnimatedPackage size={38} className="mx-auto text-zinc-300" /><p className="mt-4 text-sm font-bold">No encontramos productos</p><p className="mt-1 text-xs text-zinc-400">Prueba otra búsqueda o categoría.</p></div>}
      </div>
    </section>
