...
export default function RestauranteTemplate({ perfil, productos, extensionData, isReadOnly }: Props) {
  ...
  const handleScrollToMenu = () => {
    ...
    window.scrollTo({
      top: Math.max(0, menu.getBoundingClientRect().top + window.scrollY - 76),
      left: 0,
      behavior: 'smooth',
    })
  }
...
function PaymentBrandMark({ brand }: { brand: 'visa' | 'mastercard' | 'yape' | 'plin' | 'cash' }) {
  const marks = {
    visa: <svg viewBox="0 0 60 26" className="h-6 w-11" aria-hidden="true"><text x="30" y="19" textAnchor="middle" fontFamily="Arial,sans-serif" fontSize="18" fontWeight="900" fontStyle="italic" letterSpacing="-1.5" fill="#1A1F71">VISA</text></svg>,
    ...
    yape: <svg viewBox="0 0 42 28" className="h-7 w-10" aria-hidden="true"><circle cx="31" cy="5.5" r="4.8" fill="#25D6C4" /><circle cx="31" cy="5.5" r="1.25" fill="#EAFBF8" /><text x="20" y="23" textAnchor="middle" fontFamily="Arial,sans-serif" fontSize="15" fontWeight="900" fontStyle="italic" letterSpacing="-1.8" fill="#6F3298">yape</text></svg>,
    plin: <svg viewBox="0 0 40 28" className="h-7 w-9" aria-hidden="true"><rect x="2" y="3" width="36" height="22" rx="11" fill="#12B6A5" /><text x="20" y="18" textAnchor="middle" fontFamily="Arial,sans-serif" fontSize="11" fontWeight="900" letterSpacing="-1.1" fill="#fff">plin</text></svg>,
    cash: <svg viewBox="0 0 40 28" className="h-6 w-9" aria-hidden="true"><rect x="2" y="4" width="36" height="20" rx="3.5" fill="#E0ECDC" stroke="#9DBB9B" strokeWidth="1.5" /><path d="M7 10h4M29 18h4" stroke="#6F9D6D" strokeWidth="1.5" strokeLinecap="round" /><circle cx="20" cy="14" r="5" fill="#9BC29A" /><text x="20" y="17.2" textAnchor="middle" fontFamily="Arial,sans-serif" fontSize="8" fontWeight="800" fill="#4B7A4B">$</text></svg>,
  }
