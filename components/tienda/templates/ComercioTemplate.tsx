...
export default function ComercioTemplate({ perfil, productos, isReadOnly = false }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#F4F3EF] text-[#131A22] selection:bg-[#78DDBC] selection:text-[#131A22]">
      ...
      <main>
        ...
        ].map(([Icon, title, copy], index) => { const ServiceIcon = Icon as typeof AnimatedPackage; return <div key={title as string} className={`flex items-center gap-3 border-black/[0.07] px-3 py-5 lg:px-6 ${index % 2 === 0 ? 'border-r' : ''} lg:border-r lg:last:border-r-0`}><ServiceIcon size={23} duration={0.55} className="shrink-0 text-[#2A987B]" /><div><p className="text-[11px] font-bold">{title as string}</p><p className="mt-0.5 text-[9px] text-zinc-400">{copy as string}</p></div></div> })}</div></section>

        <section className="bg-[#131A22] text-white"><div className="mx-auto grid max-w-[1480px] grid-cols-2 divide-x divide-white/10 px-4 sm:px-6 lg:grid-cols-[1.35fr_repeat(3,1fr)]"><div className="col-span-2 flex items-center gap-4 border-b border-white/10 py-6 lg:col-span-1 lg:border-b-0 lg:pr-8"><span className="relative flex h-3 w-3 shrink-0"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#78DDBC] opacity-40" /><span className="relative inline-flex h-3 w-3 rounded-full bg-[#78DDBC]" /></span><div><p className="text-[9px] font-bold uppercase tracking-[0.18em] text-[#78DDBC]">Catálogo operativo</p><p className="mt-1 text-xs text-white/50">Información clara para decidir y comprar sin fricción.</p></div></div>{[[availableProducts, 'productos disponibles'], [brands.length, 'marcas en catálogo'], [offers, 'ofertas activas']].map(([value, label]) => <div key={label as string} className="px-5 py-6 lg:px-8"><strong className="block text-2xl font-black tracking-[-0.05em]">{value}</strong><span className="mt-1 block text-[8px] font-semibold uppercase tracking-[0.13em] text-white/40">{label}</span></div>)}</div></section>

        <ProductGrid productos={productos} perfil={perfil} isReadOnly={isReadOnly} />

        {categories.length > 1 && <section id="ofertas" className="pb-16"><div className="mx-auto max-w-[1480px] px-4 sm:px-6"><div className="grid gap-4 lg:grid-cols-2">{categories.slice(0, 2).map((category, index) => { const product = productos.find((item) => item.category === category && item.image_url); return <a key={category} href="#catalogo" onClick={() => window.dispatchEvent(new CustomEvent('commerce-category', { detail: category }))} className={`group relative min-h-[270px] overflow-hidden rounded-2xl p-8 transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(19,26,34,0.12)] ${index === 0 ? 'bg-[#131A22] text-white' : 'bg-[#DFE5E2] text-[#131A22]'}`}><div className="relative z-10 max-w-xs"><p className={`text-[9px] font-bold uppercase tracking-[0.18em] ${index === 0 ? 'text-[#78DDBC]' : 'text-zinc-500'}`}>Explora la colección</p><h2 className="mt-4 text-3xl font-black tracking-[-0.045em]">{category}</h2><span className="mt-6 inline-flex items-center gap-2 text-xs font-bold">Ver productos <AnimatedChevron size={15} duration={0.45} /></span></div>{product?.image_url && <Image src={product.image_url} alt="" fill className="object-contain object-right p-6 pl-[45%] transition-transform duration-700 group-hover:scale-105" sizes="50vw" />}</a>})}</div></div></section>}
      </main>

      <footer id="pagos" className="bg-[#131A22] text-white"><div className="mx-auto grid max-w-[1480px] gap-10 px-6 py-14 md:grid-cols-2 lg:grid-cols-4"><div><p className="text-xl font-black uppercase tracking-[-0.04em]">{storeName}</p><p className="mt-4 max-w-xs text-xs leading-6 text-white/45">Catálogo especializado, atención directa y productos con disponibilidad actualizada.</p></div><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#78DDBC]">Categorías</p><div className="mt-4 grid gap-2">{categories.slice(0, 5).map((category) => <a key={category} href="#catalogo" className="text-xs text-white/50 transition-colors hover:text-white">{category}</a>)}</div></div><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#78DDBC]">Información</p><div className="mt-4 grid gap-2 text-xs text-white/50"><span>Garantía de productos</span><span>Condiciones de entrega</span><span>Atención al cliente</span><span>Privacidad y seguridad</span></div></div><PaymentTrustBadges mercadopagoActive={perfil.mercadopago_active === true} className="text-white" /></div><div className="border-t border-white/10 px-6 py-5 text-center text-[9px] uppercase tracking-[0.14em] text-white/30">© {new Date().getFullYear()} {storeName}. Todos los derechos reservados.</div></footer>
    </div>
  )
}
