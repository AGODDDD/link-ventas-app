...
export default function SlideOverCart({ storeId, isOpen, onClose, onCheckout, templateType }: Props) {
  ...
  return (
    <>
      ...
      <aside aria-hidden={!isOpen} className={`fixed inset-y-0 right-0 z-[101] flex w-full max-w-[460px] flex-col border-l transition-transform duration-500 ease-out ${isSoftTheme ? 'border-zinc-200/80 bg-[#F4F3EF] text-[#131A22] shadow-[-30px_0_80px_rgba(0,0,0,0.15)]' : 'border-outline bg-background text-on-background'} ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <header className={`flex items-center justify-between border-b px-6 py-5 ${isSoftTheme ? 'border-zinc-900/10' : 'border-outline'}`}>
          <div className="flex items-center gap-3">
            <span className={`flex h-10 w-10 items-center justify-center rounded-xl ${isCommerce ? 'bg-[#131A22] text-[#78DDBC]' : isSoftTheme ? 'bg-white text-zinc-900' : 'bg-surface-variant text-primary'}`}><AnimatedCart size={20} duration={0.5} /></span>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          {!cart.length ? (
            <div className="flex h-full flex-col items-center justify-center px-8 text-center">
              ...
              <button type="button" onClick={onClose} className={`mt-7 rounded-xl px-5 py-3 text-xs font-semibold transition-all duration-300 active:scale-95 ${isCommerce ? 'bg-[#131A22] text-white hover:bg-[#25313D]' : isSoftTheme ? 'border border-zinc-200 bg-white text-zinc-700' : 'bg-primary text-on-primary'}`}>Ver productos</button>
            </div>
          ) : (
        ...
        {cart.length > 0 && (
          <footer className={`border-t p-6 ${isSoftTheme ? 'border-zinc-900/10 bg-white' : 'border-primary/20 bg-surface-container-high'}`}>
            ...
            <button type="button" onClick={checkout} className={`group mt-5 flex h-14 w-full items-center justify-between rounded-xl px-5 text-sm font-semibold transition-all duration-300 active:scale-[0.98] ${isCommerce ? 'bg-[#131A22] text-white hover:bg-[#25313D]' : isSoftTheme ? 'bg-zinc-900 text-white hover:bg-zinc-700' : 'bg-gradient-to-r from-primary to-secondary text-white'}`}><span>Continuar al pago</span><span className={`flex h-8 w-8 items-center justify-center rounded-lg ${isCommerce ? 'bg-[#78DDBC] text-[#131A22]' : 'bg-white/10'}`}><AnimatedChevron size={17} duration={0.45} className="transition-transform group-hover:translate-x-0.5" /></span></button>
